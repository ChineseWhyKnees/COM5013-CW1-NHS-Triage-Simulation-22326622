...
This script brings together all of the components of the NHS triage
simulation into a working command-line application. It creates and
connects the various data structures (queues, stacks, hash map, circular
buffer) and provides a simple text interface for administrators to
search for patients, manage queues, discharge and admit patients, and
run diagnostic tests. The program runs interactively until the user
chooses to log out.

At startup the script raises the Python recursion limit to avoid errors
when sorting large numbers of patients. It then constructs the data
generator and factory, initialises the database and queues, and
generates an initial batch of patients. Throughout the session the
administrator can view system statistics, move patients into different
queues and examine logs of recent actions. A special diagnostic
function allows the user to time operations and verify that the
underlying data structures perform as expected.
"""

# Imports all the custom classes
...
# Simple authentication prompt. The system is locked after 3 failed attempts.
def system_login() -> bool:
    """Simulates a secure login prompt."""
    print_header("NHS TRIAGE SYSTEM")
    print("Authentication Required. (Try: SysAd / 2232)")
    ...
    while attempts < 3:
        ...
        if user == "SystemAdmin" and pwd == "22326622":
            print(f"\n{Colors.GREEN}ACCESS GRANTED.{Colors.ENDC} Initialising environment...")
            time.sleep(0.5)
            return True
        else:
...
def run_diagnostics():
    """
    Run a suite of timing tests on the underlying data structures.
    These diagnostics seek to confirm the performance characteristics
    of the systems building blocks. The function measures and displays:

    * How long merge sorting takes for lists of various sizes. The times
      should grow roughly in proportion to ``n log n``.
    * How long it takes to dequeue 10,000 items from a custom linked queue
      compared with Python's list using ``pop(0)``.
    * How the circular queue behaves when filled, overflowed and emptied
      again, demonstrating correct wrap-around and underflow handling.
    * Whether the hash map correctly stores and retrieves items when
      intentionally overfilled to force collisions.

    The results are printed to the console for inspection. This function
    does not modify any of the main program's data structures.
    """
    ...
    print("-" * 40)
    cq = circularqueue(5)
    print("Initialising Buffer (Size=5)... OK")
...
def main():
    ...
    print_header("System Boot: Initialising Hospital Framework")
    loading_bar("Loading Patient Generator", 0.3)
    loading_bar("Initialising Database", 0.3)
    ...
    # 2. Generate Initial Data (50,000+ patients works just be patient)
    ...
    # 3. Interactive Menu
    while True:
        print_header("HOSPITAL ADMINISTRATION SYSTEM (HAS)")
        ...
        if choice == '1':
            ...
            if record:
                ...
                if "Discharged" in current_status:
                    ...
                    # Logic fix: Only allow action if patient is not discharged
                    if action == '1':
            ...
            if input("\nProcess Queues? (y/n): ").lower() == 'y':
                ...
                if not blood_lab_buffer.is_empty():
                    ...
                    p.update_status("Blood Analysed")
            ...
            while not blood_lab_buffer.is_empty():
                p = blood_lab_buffer.dequeue()
                p.update_status("Lab Results Analysed")
                print(f"[{p.nhs_number}]", end=" ")
                lab_count += 1
            print(f"({lab_count} samples analysed)")
